...
describe('Utils', () => {
  test('preparePrompt with selection', () => {
    ...
    expect(result.prompt).toBe('Process this: Selected text\n\nContext:\nSome context');
  });
  ...
  test('preparePrompt with context keyword', () => {
    ...
    expect(result.prompt).toBe('Process this with context: Some context\n\nSelected text');
  });

  test('preparePrompt with empty prompt', () => {
    const result = preparePrompt('', 'Selected text', 'Some context');
    expect(result.prompt).toBe('Selected text\n\nContext:\nSome context');
  });
  ...
  test('preparePrompt with multiple context conditions', () => {
    ...
    const result = preparePrompt(prompt, 'Selected text', 'Some context');
    expect(result.prompt).toBe(`${CONTEXT_CONDITION_START}Start${CONTEXT_CONDITION_END} Middle ${CONTEXT_CONDITION_START}End${CONTEXT_CONDITION_END}\n\nSelected text\n\nContext:\nSome context`);
  });
  ...
  test('preparePrompt with mismatched context conditions', () => {
    ...
    const result = preparePrompt(prompt, 'Selected text', 'Some context');
    expect(result.prompt).toBe(`${CONTEXT_CONDITION_START}Incomplete ${CONTEXT_CONDITION_START}Condition${CONTEXT_CONDITION_END}\n\nSelected text\n\nContext:\nSome context`);
  });
  ...
  test('preparePrompt with selection and context keywords in reverse order', () => {
    ...
    const result = preparePrompt(prompt, 'Selected text', 'Some context');
    expect(result.prompt).toBe('Context: Some context\nSelection: Selected text');
  });
});